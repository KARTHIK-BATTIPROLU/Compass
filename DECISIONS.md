...
### DEC-021: The 3-key Gemini fallback chain shares one quota pool — confirmed empirically
...
- **Impact:** DEC-009's "resilient 3-layer LLM factory" protects against a single key being transiently unhealthy, but provides **no actual protection against quota exhaustion** — which is the failure mode that matters most on a free tier this small (20 req/day is trivial to exhaust while testing). This isn't fixable in code; it needs either a paid tier, or fallback keys provisioned under genuinely separate GCP projects/billing accounts.
- **Consequence for this sprint:** B2 (quiz nudge) and later turns of the B3 drill-down test could not be fully re-verified end-to-end after quota exhausted mid-session — see VERIFICATION.md for exactly what was and wasn't directly observed working.
