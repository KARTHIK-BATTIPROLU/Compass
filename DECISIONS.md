...
### DEC-025: Migration 002 applied — verified cross-user isolation, found and fixed a gap it left in the public quiz path
- User applied `002_rls_and_artifact_types.sql` via the Supabase SQL Editor. Re-verified empirically:
  - **Owner reads work again:** the same "authenticated user reading their own `sessions` row" query from DEC-024 now returns the real row (previously `[]`).
  - **Cross-user isolation confirmed:** a second, unrelated user's JWT querying the first user's known session by id returns `[]` — this is the actual security property A2 exists for, now proven, not just asserted.
  - **`artifacts.type` CHECK fix confirmed:** inserting a row with `type='resource_card'` now succeeds (`201`) — this used to violate the constraint (see the harness findings under "Final Sprint — Harness Run 1").
  - **Gap found:** anon-key reads of `quizzes` (no JWT — this is exactly what the public `/q/[token]` page does, since respondents are never logged in) returned `[]`. Migration 002 only granted SELECT to the owning teacher; it never accounted for the public share-link path needing read access too. This is a hole in the migration's own design surfacing now that RLS is actually enforced, not a re-emergence of the original problem.
- **Fix:** `003_public_quiz_read.sql` adds `quizzes_public_read_open`, scoping public SELECT to `open = true` quizzes. `share_token` is the real bearer secret (a hard-to-guess UUID) — RLS has no concept of "only when queried by the matching token," so this matches the access level `routers/quiz.py`'s existing service-key-backed `GET /api/quiz/{token}` already grants, just expressed as a row policy. Folded into `schema.sql` too. Asked the user to apply this one additional two-line policy the same way.
